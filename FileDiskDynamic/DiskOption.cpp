...
BOOLEAN __stdcall FormatExCallback(CALLBACKCOMMAND Command, DWORD Modifier, PVOID Argument)
{
	...
	case PROGRESS:
		...
		sprintf(strBuffer, "%d percent completed.\n", *percent);
	...
	case DONE:
		...
		if (*status == FALSE) {
			...
			sprintf(strBuffer, "FormatEx was unable to complete successfully.\n");
			OutputDebugStringA(strBuffer);
			Error = TRUE;
		}
		break;
...
// 	RootDirectory[0] = L'G';
	...
	if (!LoadFMIFSEntryPoints()) {
		...
		OutputDebugStringA("Could not located FMIFS entry points.\n");
		return -1;
	}
